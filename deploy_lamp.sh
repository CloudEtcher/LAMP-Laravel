...
sudo mysql -uroot -e "CREATE DATABASE Eniola;"
sudo mysql -uroot -e "CREATE USER 'etcher'@'localhost' IDENTIFIED BY 'cloud';"
sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON Eniola.* TO 'etcher'@'localhost';"
cd /var/www/laravel
...
sudo sed -i '25 s/=laravel/=Eniola/' /var/www/laravel/.env
sudo sed -i '26 s/=root/=etcher/' /var/www/laravel/.env
sudo sed -i '27 s/=/=cloud/' /var/www/laravel/.env
